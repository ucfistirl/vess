...
// NOTE:  This implementation uses Performer and MS Windows functions
...
vsWindowSystem::vsWindowSystem(vsWindow *mainWindow)
{
    ...
    // Get the MS Window and what Performer calls the Drawable from the 
    // pfPipeWindow associated with the vsWindow.  We need to subclass both of 
    // these to get all the events we want.
    window = mainWindow->getBaseLibraryObject()->getWSWindow();
    drawable = mainWindow->getBaseLibraryObject()->getWSDrawable();
    ...
    // Subclass the MS Windows handles associated with the vsWindow to install an
    // additional window procedure to handle input events.  First try what Performer
    // calls the WSDrawable, which can be a HANDLE to either a window or a pbuffer.
    // Make sure the drawable is a window before trying to subclass it
    if (IsWindow((HWND)drawable))
    {
        originalDrawableProc = (WNDPROC)SetWindowLongPtr((HWND)drawable, GWLP_WNDPROC, 
            (LONG_PTR)inputWindowProc);
    }
    else
    {
        // This isn't a real window
        printf("vsWindowSystem::vsWindowSystem:  Given vsWindow is not a valid "
            "window!\n");
        drawable = 0x0;
    }

    // Now, subclass the WSWindow, which is always a real window handle
    originalWindowProc = (WNDPROC)SetWindowLongPtr(window, GWLP_WNDPROC, 
        (LONG_PTR)inputWindowProc);

    // Register the window with the window system, so we can look it up when
    // processing events later.
...
vsWindowSystem::~vsWindowSystem()
{
    ...
    // Restore the original window procedure to the MS Window
    SetWindowLongPtr(window, GWLP_WNDPROC, (LONG_PTR)originalWindowProc);

    // Check the drawable for validity and restore its procedure as well
    if (drawable != 0x0)
        SetWindowLongPtr((HWND)drawable, GWLP_WNDPROC, (LONG_PTR)originalDrawableProc);
                                                 ...
                                                 WPARAM wParam, LPARAM lParam)
{
    ...
    // Get the VESS window and window system from the msWindow parameter.  First
    // try the window map.
    vessWindow = (vsWindow *)vsWindow::getWindowMap()->mapFirstToSecond(msWindow);
  
    // If the MS Window was not found in the window map, try the drawable map
    if (vessWindow == NULL)
    {
        vessWindow = (vsWindow *)vsWindow::getDrawableMap()->
            mapFirstToSecond(msWindow);
    }
    ...
    // Get the window system object associated with this vsWindow
    ...
    switch (message)
    {
        ...
        default:
            ...
            // by Performer).  Make sure to call the appropriate window procedure
            // depending on whether this call is for the WSWindow or WSDrawable
            if ((windowSys->drawable != 0x0) && (msWindow == windowSys->drawable))
            {
                // Call drawable's original window procedure, if the drawable is
                // valid
                return CallWindowProc(windowSys->originalDrawableProc, msWindow,
                    message, wParam, lParam);
            }
            else if (msWindow == windowSys->window)
            {
                // Call window's original window procedure
                return CallWindowProc(windowSys->originalWindowProc, msWindow, 
                    message, wParam, lParam);
            }
            else
            {
                // This shouldn't happen.  The msWindow parameter should match one
                // of the two windows above (window or drawable), but it doesn't.
                // Call the default window procedure to avoid losing the event.
                printf("vsWindowSystem::inputWindowProc:  WARNING -- Calling "
                    "default window procedure for unrecognized window\n");
                return DefWindowProc(msWindow, message, wParam, lParam);
            }
    }
