...
vsEthernetMotionStar::~vsEthernetMotionStar()
{
    ...
    // Check if we've spawned a server thread
    if (forked)
    {
...
// ------------------------------------------------------------------------
DWORD WINAPI vsEthernetMotionStar::serverLoop(void *parameter)
{
    ...
    // Constantly update the private tracker data from hardware
    ...
    printf("vsEthernetMotionStar server thread exiting...\n");
    return 0;
}
...
// be run in a separate thread when using this mode.
// ------------------------------------------------------------------------
...
void vsEthernetMotionStar::setActiveHemisphere(int trackerNum, short hSphere)
{
    ...
    if ((master) && (hSphere >= 0) && (hSphere <= 5))
    {
        ...
        if (trackerNum == VS_MSTAR_ALL_TRACKERS)
        {
            ...
            for (index = 0; index <= numTrackers; index++)
            {
                trackerConfig[index].hemisphere = (unsigned char)hSphere;
            }
        }
        else
        {
            ...
            if ((trackerNum >= 0) && (trackerNum < numTrackers))
            {
                trackerConfig[trackerNum].hemisphere = (unsigned char)hSphere;
            }
        }
    }
}
