...
// ------------------------------------------------------------------------
bool vsSphereIntersect::intersectWithBox(vsSphere sphere, pfBox *box)
{
    double sqrDist;
    double sqrRadius;
    vsMatrix invXform;
    ...
    if (centerX < box->min[PF_X])
        sqrDist += VS_SQR(centerX - box->min[PF_X]);
    else if (centerX > box->max[PF_X])
        sqrDist += VS_SQR(centerX - box->max[PF_X]);
  
    // Check the y-axis, same procedure
    if (centerY < box->min[PF_Y])
        sqrDist += VS_SQR(centerY - box->min[PF_Y]);
    else if (centerY > box->max[PF_Y])
        sqrDist += VS_SQR(centerY - box->max[PF_Y]);

    // Check the z-axis, same procedure
    if (centerZ < box->min[PF_Z])
        sqrDist += VS_SQR(centerZ - box->min[PF_Z]);
    else if (centerZ > box->max[PF_Z])
        sqrDist += VS_SQR(centerZ - box->max[PF_Z]);
...
void vsSphereIntersect::intersectSpheres(vsNode *targetNode)
{
    ...
    if (targetNode->getNodeType() == VS_NODE_TYPE_GEOMETRY)
    {
        ...
        // graphics API for this).  First get the pfGeode from the
        // vsGeometry.
        performerGeode = ((vsGeometry *)targetNode)->getBaseLibraryObject();
       
        // There should only be one drawable in the Geode (this is how
        // the vsDatabaseLoader works), print a warning if this isn't so
        if (performerGeode->getNumGSets() > 1)
        {
            printf("vsSphereIntersect::intersectSpheres:  pfGeode has "
                "more than 1 pfGeoSets!\n");
        }

        // See if the intersector's bounding sphere intersects with the 
        // bounding box.  If not, we need go no farther. 
        performerBox = new pfBox;
        boundMode = performerGeode->getGSet(0)->getBound(performerBox);
        if (!intersectWithBox(boundSphere, performerBox))
        {
            return;
        }
        ...
        {
            if (intersectWithBox(*(vsSphere *)sphereList[i], performerBox))
            {
                intersectWithGeometry(i, (vsGeometry *)targetNode);
            }
        }

        // Delete the pfBox (we don't need it anymore)
        delete performerBox;
    }
    else
    {
