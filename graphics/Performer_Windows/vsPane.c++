...
{
    performerChannel->setLODAttr(PFLOD_SCALE, (float)newScale);
}
...
{
    return performerChannel->getLODAttr(PFLOD_SCALE);
}
...
void vsPane::updateView()
{
    ...
    sceneView->getClipDistances(&nearClip, &farClip);
    if ((curNearClip != nearClip) || (curFarClip != farClip))
    {
	    performerChannel->setNearFar(nearClip, farClip);
