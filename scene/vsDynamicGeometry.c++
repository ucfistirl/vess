...
vsDynamicGeometry::~vsDynamicGeometry()
{
    ...
    // Remove the pfGeode link from the object map
...
// drawn.
// ------------------------------------------------------------------------
void vsDynamicGeometry::setVisibilityValue(unsigned int newValue)
{
    // Set the mask of the Performer draw traversal for this node to 
    // the given value
...
// ------------------------------------------------------------------------
unsigned int vsDynamicGeometry::getVisibilityValue()
{
    // Get the current draw traversal mask for this node from Performer and
    // return it
...
// ------------------------------------------------------------------------
void vsDynamicGeometry::inflateFlatGeometry()
{
    int loop, primLoop;
    int jumpCount;
    ...
    // Figure out the total length of the primitives
    listSum = 0;
    for (loop = 0; loop < getPrimitiveCount(); loop++)
        listSum += lengthsList[loop];
    
    // If the color binding is per-vertex, inflate the color list
    ...
    // If the normal binding is per-vertex, inflate the normal list
...
// ------------------------------------------------------------------------
vsNode *vsDynamicGeometry::nodeSearch(const char *name, int *idx)
{
    // See if the name matches
    if (!strcmp(name, getName()))
    {
        // Name matches, so check to see if the index is zero
        if ((*idx) > 0)
        {
            // Index is not zero, decrement the index, and return NULL
            // so that recursive searches can backtrack and continue.
            (*idx)--;
            return NULL;
        }
        else
    ...
    // No match, return NULL so that recursive searches can backtrack
    // and continue
    return NULL;
}
...
void vsDynamicGeometry::applyAttributes()
{
    ...
    // Use the pfGeoState attached to the pfGeoSet in this object
    // to set the parameters of the current vsGraphicsState
